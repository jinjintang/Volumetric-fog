...
namespace vks
{
	...
	struct Model {
		...
		*/
		bool loadFromFile(const std::string& filename, vks::VertexLayout layout, vks::ModelCreateInfo *createInfo, vks::VulkanDevice *device, VkQueue copyQueue,glm::vec4 albedo=glm::vec4(0))
		{
			...
			if (pScene)
			{
				...
				for (unsigned int i = 0; i < pScene->mNumMeshes; i++)
				{
					...
					aiColor3D pColor(albedo.x, albedo.y, albedo.z);
					...
					for (unsigned int j = 0; j < paiMesh->mNumVertices; j++)
					{
						...
						for (auto& component : layout.components)
						{
							...
							case VERTEX_COMPONENT_DUMMY_FLOAT:
								...
								break;
							case VERTEX_COMPONENT_DUMMY_VEC4:
								vertexBuffer.push_back(albedo.r);
								vertexBuffer.push_back(albedo.g);
								vertexBuffer.push_back(albedo.b);
								vertexBuffer.push_back(albedo.a);
								break;
							};
						}
		...
		* @param copyQueue Queue used for the memory staging copy commands (must support transfer)
		*/
		bool loadFromFile(const std::string& filename, vks::VertexLayout layout, float scale, vks::VulkanDevice *device, VkQueue copyQueue,glm::vec4 albedo=glm::vec4(0.0))
		{
			vks::ModelCreateInfo modelCreateInfo(scale, 1.0f, 0.0f);
			return loadFromFile(filename, layout, &modelCreateInfo, device, copyQueue,albedo);
		}
	};
};